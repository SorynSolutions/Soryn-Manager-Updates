...
class MacroManager {
  ...
  executeRandomMovement() {
    ...
    try {
      ...
      // Use the simplest approach: createElement + dispatchEvent
      synchronizedViews.forEach(view => {
        if (view.webContents && !view.webContents.isDestroyed()) {
          const script = `
            (function() {
              try {
                ...
                setTimeout(() => {
                  ...
                  // Correction for backward movement (only if there was no combination)
          ...
          view.webContents.executeJavaScript(script)
            ...
            .catch(err => {
              ...
              // Try an alternative approach in case of error
