class SoundController {
  constructor() {
    ...
    this.volume = 100; // Default volume
  }
  ...
  // Set up the gain node for a media element
  setupGainNode($media, audioStream) {
    ...
    try {
      ...
      console.error("Error setting up gain node:", error);
      this.audioGainNode = null;
    }
  }

  // Set the gain node volume
  ...
  toggleMute() {
    if (this.audioGainNode) {
      // Use the gain node if available
...
// Class to manage the audio control UI
class AudioUI {
  constructor() {
